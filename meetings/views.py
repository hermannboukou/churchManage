...
def graphique_annuel(request):
    ...
    meeting_id = request.GET.get('meeting_type')
    year = request.GET.get('year', timezone.now().year)
    church_id = request.GET.get('church')

    # Valeurs par défaut intelligentes
    # Par défaut, type de réunion = 'Culte d\'adoration' si existe
    default_meeting = meetings.filter(name__icontains="culte d'adoration").first()
    default_meeting_id = default_meeting.id if default_meeting else (meetings.first().id if meetings.exists() else None)
    # Par défaut, première église
    default_church_id = churches.first().id if churches.exists() else None
    ...
    years_qs = Audiences.objects.dates('day', 'year').values_list('day__year', flat=True).distinct()
    years = set(years_qs)
    # Toujours inclure l'année en cours dans la liste
    ...
    context = {
        ...
        'churches': churches,
        'years': years,
        'selected_meeting_type': int(meeting_id) if meeting_id and str(meeting_id).isdigit() else '',
        'selected_church': int(church_id) if church_id and str(church_id).isdigit() else '',
    ...
    for meeting in meetings_for_chart:
        for i, category in enumerate(categories):
            ...
            for month_num in range(1, 13):
                ...
                # Calculer la moyenne mensuelle (et non le cumul)
                qs = Audiences.objects.filter(q_filter)
                count = qs.count()
                monthly_total = qs.aggregate(total=Sum(category))['total'] or 0
                monthly_avg = (monthly_total / count) if count else 0
                dataset['data'].append(round(monthly_avg, 2))
...
def audience_report(request):
    ...
    context = {
        ...
        'churches': churches,
        'meetings': meetings,
        'selected_church': int(church_id) if church_id else None,
        'selected_meeting': int(meeting_id) if meeting_id else None,
